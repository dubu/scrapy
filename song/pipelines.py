...
class SongPipeline(object):
    def __init__(self):
        self.conn = MySQLdb.connect(user='scrapy', passwd='scrapy', db='scrapy', host='localhost', charset='utf8', use_unicode=True)
    ...
    def process_item(self, item, spider):
        try:
            ...
            self.cursor.execute('''INSERT INTO scrapy_football_player (name, nationality, birthday, birthplace, height, weight, position, currentClub, number) VALUES (%s, %s,%s, %s, %s, %s, %s, %s, %s)''', (name, nationality, birthday, birthplace, height, weight, position, currentClub, number))
            self.conn.commit()
